...
function! vimupdate#Update() abort
  ...
  let check_result = system($'powershell.exe -NoProfile -ExecutionPolicy Bypass -File {ps_script} -CurrentVersionLong {v:versionlong} -VimRuntime {shellescape($VIMRUNTIME)} -Checkonly')->trim()
